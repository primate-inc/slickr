...
export default class extends Controller {
  ...
  getResults(filters, page) {
    ...
    .then(function (response) {
      const events_listing = document.getElementById('events_listing')
      ...
      if(page > 1) {
        const load_more_button = document.getElementById('events_load_more')
        load_more_button.remove()

        // Save visible elements and add new ones along with saved
        const elState = events_listing.innerHTML
        events_listing.innerHTML = elState + response.data
      } else {
        events_listing.innerHTML = response.data
      }
